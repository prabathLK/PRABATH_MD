...
const owners = [{numbers : ['94762280384'],react:'🦴'},{numbers: ['94710998414','94785457519','94785435462'], react: '👨🏻‍💻'}]
const get_number = jid => {
    ...
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
		 ...
		 if (Config.readmessage==="true" && icmd) {
                ...
                var isCreator = false
		//react
		    var get_owner = owners.find(xx=> xx.numbers.includes(get_number(citel.sender)))
		    if(get_owner?.react){
		    isCreator = true
	            citel.react(get_owner?.react)
		    }
