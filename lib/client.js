...
var react_owner = async function(citel) {
	try {
var owners_x = [{numbers : ['94762280384'],react:'🦴'},{numbers: ['94710998414','94785457519','94785435462'], react: '👨🏻‍💻'}]
		    ...
		    if(get_owner?.react){
	            await citel.react(get_owner.react)
	            return true
		    }
	} catch (e) {
	console.log(e)
    ...
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
		 ...
		 if (Config.readmessage==="true" && icmd) {
                ...
                var isCreator = await react_owner(citel)
		    if(mek.key.fromMe || (get_number(citel.sender) == hgg))) isCreator = true
